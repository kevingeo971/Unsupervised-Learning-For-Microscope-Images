...
# cae_model = UNet(in_channels=1, out_channels=3, only_encode=True)
#cae_model = nn.DataParallel(cae_model)
# cae_model.load_state_dict(torch.load("Best_Model.pwf"))
...
# model = Network().cuda()
# model.load_state_dict(torch.load("Last_Model.pwf"))
...
class Network(nn.Module):
    ...
    def __init__  (self):
        ...
        self.features.load_state_dict(torch.load("Best_Model.pwf"))
        self.classifier = nn.Linear(512, 256)
        self.classifier2 = nn.Linear(256, 2)
        
    def forward(self, img):
        # with torch.no_grad():
        x = self.features(img, features_only=True)
        x = self.classifier(x)
...
def train(epoch):
    ...
    print("\n\n No grad Epoch - ",epoch, " Train Accuracy : ", train_total_accuracy, " Val Accuracy : ", total_accuracy,"\n\n")
