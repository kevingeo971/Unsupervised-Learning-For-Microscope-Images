...
X_train_low_high = np.stack([X_train_lowF,X_train_highF]).transpose(1,2,3,0)
X_val_low_high = np.stack([X_val_lowF,X_val_highF]).transpose(1,2,3,0)

print("Shape : ",X_train_og.shape, X_train_low_high.shape)
...
class CancerData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        high_low_batch = self.Y_data[idx]
        return self.transform_image(og_batch), self.transform_image(high_low_batch)

Data_train = CancerData(X_train_og, X_train_low_high)
Data_val = CancerData(X_val_og, X_val_low_high)
...
X_batch , X_batch_high_low = next(iter(data_loader_val))
print("\n\nOne Batch : ",type(X_batch_high_low), X_batch_high_low.shape, X_batch_high_low.numpy().max(),X_batch_high_low.numpy().min())
...
def validate(epoch):
    ...
    output_low = (output[0,0].cpu().numpy()*127.5 + 127.5).astype(np.int32)
    output_high = (output[0,1].cpu().numpy()**127.5 + 127.5).astype(np.int32)
    ...
    H,W,_ = image.shape
    return op_val_loss, image, output_low, output_high
...
for i in range(Epochs):
    ...
    op_val_loss,image, output_low, output_high = validate(i)
    # scheduler.step()
    ...
    if op_val_loss<best_loss:
        ...
        mim.imsave("Outputs/Best_output_low.png",output_low)
        mim.imsave("Outputs/Best_output_high.png",output_high)

    else:
        mim.imsave("Outputs/Current_image.png",image.squeeze())
        mim.imsave("Outputs/Current_output_low.png",output_low.squeeze())
        mim.imsave("Outputs/Current_output_high.png",output_high.squeeze())
